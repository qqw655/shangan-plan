...
const PHASES = [
  {
    name: "打底期",
    days: "第 1-30 天",
    tip: "行测打底 + 公专框架 + 申论概括入门 · 每日 8 小时",
    modules: [
      { name: "资料分析", min: 100, what: "广东 20 题/套：每天 3 篇材料限时（每篇 7 分钟），公式三件套+速算——主攻，目标 90%+" },
      { name: "判断逻辑", min: 65, what: "广东逻辑判断 15 题：翻译推理公式化（补 0 分项）+ 加强削弱 10 题" },
      { name: "言语理解", min: 50, what: "广东 15 题/套：逻辑填空约 5 题（背高频成语）+ 片段阅读约 10 题，限时 13 分钟" },
      { name: "数量·数推", min: 40, what: "广东 15 题=数推 5 + 运算 10：数推四类套路 5 题 + 运算挑 5 题练" },
      { name: "图推+科学", min: 35, what: "广东图推 5 + 科学推理 5：立体类优先 + 初中物理（受力/运动/功）" },
      { name: "错题+间隔", min: 40, what: "当日错题归因重做 + 上周错题重做" },
      { name: "公专", min: 90, what: "按你自己的方法推进，这里只固定时间（1.5 小时）" },
      { name: "申论·概括", min: 45, what: "广东申论不考大作文、只考小题：练单一概括题（问题/原因/对策）踩点，每周 2-3 题" },
      { name: "时政", min: 15, what: "广东常识 15 题：只跟时政 / 政治理论 / 法律，范围无边不系统学" }
    ]
  },
  {
    name: "巩固期",
    days: "第 31-75 天",
    tip: "行测提速 + 公专真题 + 申论分析/对策/公文 · 每日 8 小时",
    modules: [
      { name: "资料分析", min: 100, what: "广东 20 题限时 30 分钟提速，综合判断用排除法" },
      { name: "判断逻辑", min: 60, what: "广东逻辑 15 题真题：加强/削弱/组合排列，翻译保手感" },
      { name: "言语理解", min: 40, what: "广东 15 题限时 13 分钟，逻辑填空为重点" },
      { name: "数量·数推", min: 40, what: "数推 5 题 15 分钟保手感；运算只练高频 6 类，每题≤1.5 分钟" },
      { name: "图推+科学", min: 30, what: "图推 5 题规律类扫题；科学推理 5 题物理为主" },
      { name: "错题+间隔", min: 40, what: "按题型归档错题，重做上周错题" },
      { name: "公专", min: 100, what: "按你的方法：真题 + 错题回归知识点（1.7 小时）" },
      { name: "申论·分析对策", min: 60, what: "综合概括 + 观点/影响分析 + 对策题（针对性/可行性）；每周 1 套广东申论小题" },
      { name: "时政", min: 10, what: "广东常识 15 题：时政 + 法律 + 广东本地（大湾区/高质量发展）" }
    ]
  },
  {
    name: "套题期",
    days: "第 76-105 天",
    tip: "全真模拟为主 · 公专套题 + 申论真题 · 每日 8 小时",
    modules: [
      { name: "行测套题", min: 120, what: "隔天一套广东真题全真 90 分钟 + 复盘（按周测侧重微调）", freq: "隔天" },
      { name: "申论套题", min: 90, what: "每周 2 套广东申论真题（限时 90 分钟），踩点复盘", freq: "每周2次" },
      { name: "公专套题", min: 90, what: "整套限时 + 知识点回顾", freq: "每周2次" },
      { name: "资料保持", min: 30, what: "每天 20 题限时（广东量），保持 90%+" },
      { name: "判断+言语保持", min: 40, what: "各 15 题保持手感（广东量）" },
      { name: "错题复盘", min: 60, what: "套题错题全量复盘 + 间隔重做" },
      { name: "时政", min: 20, what: "时政 + 全会公报滚动" }
    ]
  },
  {
    name: "冲刺期",
    ...
    modules: [
      ...
      { name: "申论踩点速览", min: 45, what: "概括/分析/对策/公文格式回看，每 3 天练 1 题" },
      { name: "公专错题", min: 60, what: "公专错题回归知识点（方法自定）" },
      { name: "时政背诵", min: 45, what: "全会公报 + 时政最后冲刺" },
  ...
  train: {
    1: {
      ...
      modules: [
        ...
        { name: "公专刷题", min: 30, what: "按你的方法：公安章节题/真题 20 题 + 解析，错题回查思维导图" },
  ...
  rest: {
    3: {
      ...
      modules: [
        ...
        { name: "公专真题", min: 45, what: "合卷中的公专部分：真题 20 题 + 解析，错题回查思维导图" }
      ]
    },
    6: {
      tag: "休息日 · 8.5h",
      ...
      modules: [
        ...
        { name: "常识时政", min: 30, what: "近 1 月时政 30 条" },
        { name: "公专错题回归", min: 45, what: "本周公专错题回归思维导图知识点，重做一遍" },
    ...
    0: {
      ...
      modules: [
        ...
        { name: "公专错题回归", min: 45, what: "公专错题回归思维导图，重做错题" },
        { name: "申论小题", min: 45, what: "概括/分析/对策小题 1 道，踩点复盘（广东只考小题）" },
...
const STRATEGY = {
  gd: [
    "题量 90 题 / 90 分钟，平均每题 1 分钟，时间管理本身就是分数",
    "特色模块：科学推理（物理/化学/生物/地理）全国少有，广东必考",
    "数量关系含数字推理（多数省份没有），是广东固定送分/送命题",
    "判断推理无定义判断、无类比推理，集中考图推 + 逻辑 + 科学推理",
    "言语题量少（约 15 题），逻辑填空性价比高",
    "资料分析 20 题、分值最高，难度中等、计算量适中，最值得投入",
    "常识偏政治理论 + 时政 + 法律，范围无边，只宜跟时政",
    "申论县级只考小题（概括/分析/对策/应用文），不考大作文，踩点给分"
  ],
  ti: [
    { m: "常识+政治", n: "15", t: "8-9分钟", target: "60%", note: "只跟时政/政治理论/法律，不系统学" },
    { m: "言语理解", n: "15", t: "13-14分钟", target: "80%", note: "逻辑填空约5 + 片段阅读约10" },
    { m: "数量关系", n: "15", t: "12-15分钟", target: "65%", note: "数推5（套路必拿）+ 运算10（挑题）" },
    { m: "判断推理", n: "25", t: "22-24分钟", target: "80%", note: "图推5 + 逻辑15 + 科学推理5" },
    { m: "资料分析", n: "20", t: "30-32分钟", target: "90%", note: "主攻，分值最高" }
  ],
  focus: [
    { t: "资料分析（冲刺 90%+）", d: "广东分值最高、你实测 80% 已是强项。每天限时 20 题，先看题干再找数，综合判断用排除法——这是你提分的头号基地。" },
    { t: "数字推理（20% → 70%+）", d: "广东必考且纯套路。先练多重/机械划分/递推/图形数阵四类，每天 15 分钟，2-3 周见效，价值 4-5 分。" },
    { t: "逻辑判断·翻译推理（0 → 70%+）", d: "你这次 0/2，属于没入门。把 如果那么/只有才/除非否则 公式化，半天就能学会，之后就是白送分。" },
    { t: "科学推理（60% → 80%）", d: "广东特色 5 题。补初中物理受力分析/运动图像/功和能三块（你 3 题全错），化学/地理/生物背结论即可。" },
    { t: "逻辑填空（保持 80%+）", d: "你的强项。背桌面那份广东高频成语表，每天 15 分钟，别让强项掉。" },
    { t: "申论小题（全权规划）", d: "广东只考小题、踩点给分，短期提分空间大。按 app 里申论四阶段走：概括→分析/对策→公文→套题。" }
  ],
  drop: [
    { t: "常识系统学习（只跟时政）", d: "你政治理论 0/4 但范围无边，系统学性价比极低。每天 10-20 分钟只跟全会公报、重要文件、广东本地时政。" },
    { t: "数学运算难题", d: "排列组合/最值/概率难题直接蒙。运算只练高频 6 类（工程/行程/几何/经济利润/和差倍比/计数），每题限时 1.5 分钟，超时跳过。" },
    { t: "图推规律难题", d: "立体类（三视图/拼合/折叠）优先拿分，规律类 45 秒没思路就蒙，别死磕黑白运算这类冷门规律。" },
    { t: "言语语句排序/填空深挖", d: "保持基础正确率即可，不值得大量时间；重点保住逻辑填空。" },
    { t: "大作文", d: "广东县级申论不考大作文，不要花时间背模板练大作文。" },
  ...
  shenlun: [
    { phase: "打底期（W1-30）", d: "每周 2-3 题：单一概括题（问题/原因/对策）。先学「踩点」：读材料圈关键词→同类合并→规范表达，只练概括。" },
    { phase: "巩固期（W31-75）", d: "每周 1 套广东真题小题：综合概括 + 观点/影响分析 + 对策题（针对性/可行性）；公文写作按 通知/倡议书/讲话稿/工作建议 每周 1 种文体。" },
    { phase: "套题期（W76-105）", d: "每周 2 套广东申论真题，限时 90 分钟，逐题对答案踩点；错点归类（漏点/表达不规范/格式错）。" },
    { phase: "冲刺期（W106-117）", d: "只回顾：踩点技巧 + 公文格式 + 高频规范词；每 3 天练 1 题保持手感，不再学新。" }
  ]
};
...
const ORDER = [
  ...
  { name: "判断逻辑", num: "15题", time: "13-14分钟", tip: "削弱加强/翻译先做；单题 ≤55 秒" },
  { name: "图形推理", num: "5题", time: "4-5分钟", tip: "立体类先做，规律类 45 秒没思路就蒙" },
...
function renderStrategy() {
  $("strategyGd").innerHTML = STRATEGY.gd.map((p) => "<li>" + esc(p) + "</li>").join("");
  $("strategyTi").innerHTML = STRATEGY.ti.map((r) =>
    '<div class="ti-row"><b>' + esc(r.m) + "</b><span class=\"ti-num\">" + esc(r.n) + " 题</span>" +
    '<span class="ti-time">' + esc(r.t) + "</span><span class=\"ti-target\">目标 " + esc(r.target) + "</span>" +
    '<em>' + esc(r.note) + "</em></div>").join("");
  $("strategyFocus").innerHTML = STRATEGY.focus.map((f) =>
    '<div class="strat-item"><b>' + esc(f.t) + "</b><div>" + esc(f.d) + "</div></div>").join("");
  $("strategyDrop").innerHTML = STRATEGY.drop.map((f) =>
    '<div class="strat-item drop"><b>' + esc(f.t) + "</b><div>" + esc(f.d) + "</div></div>").join("");
  $("strategyDaily").innerHTML = STRATEGY.daily.map((d) =>
    '<div class="strat-daily"><b>' + esc(d.phase) + "</b><span>" + esc(d.split) + "</span></div>").join("");
  $("strategyShenlun").innerHTML = STRATEGY.shenlun.map((s) =>
    '<div class="strat-shen"><b>' + esc(s.phase) + "</b><div>" + esc(s.d) + "</div></div>").join("");
}
